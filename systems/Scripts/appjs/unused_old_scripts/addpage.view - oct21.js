...
$(document).ready(function () {
    ...
    $("#available-modules").sortable({
        ...
        receive: function (event, ui) {
            ...
            if (ui.item.hasClass("sorted-module")) {
                // This is a new item
                ui.item.removeClass("sorted-module");
                ui.item.addClass("available-module");
                ui.item.data("id", ui.sender.attr("id"));
            }
        }
     }); // sortable
         ...
         stop: function (event, ui) {
           // alert("[" + this.id + "] received [" + ui.item.html() + "] from [" + ui.sender.attr("id") + "]");
            // $(ui.helper).attr("id", "new-id");
             if (ui.item.hasClass("available-module")) {
                 // This is a new item
                 ui.item.removeClass("available-module");
                 ui.item.addClass("sorted-module");
                // ui.item.data("sorted-id", ui.sender.attr("id"));
                // ui.item.data("sorted-id", ui.sender.attr("id"));
             }
         }
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (i, value) {
                ...
                var moduleHTML = '<li class="ui-widget-content ui-corner-tr module available-module" id="' + value.FacultyProfileModuleId + '" data-modid="' + value.FacultyProfileModuleId + '" value="' + value.FacultyProfileModuleId + '">' +
